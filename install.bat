...
if not exist .env (
    ...
    python -c "import secrets, pathlib; pathlib.Path('.env').write_text(pathlib.Path('.env.example').read_text().replace('change-me-to-a-long-random-string', secrets.token_hex(32)))"
) else (
